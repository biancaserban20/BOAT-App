...
export default function ClientDetails() {
  ...
  const [usernameClient, setUsernameClient] = useState(localStorage.getItem("user-name-client"));
  const [imageLinkClient, setImageLinkClient] = useState("");
  ...
  const getInfo = () => {
      ...
      axios.get("http://localhost:8080/accounts/getImageLink", { params: {
          ...
          .then((result) => {
            ...
            console.log("res is", result.data);
            console.log("res is", result.status);
            setImageLinkClient(result.data);
          })
          .catch(error => {
            if (error.response)
              console.log("error is", error.response.data);
  ...
  return(
    ...
    <Paper className={classes.paper} elevation={3} style={paperStyle}>
          <div className="client-profile">
            <div>
            <Avatar src={imageLinkClient} sx={{ width: 180, height: 180 }}/>
            </div>
